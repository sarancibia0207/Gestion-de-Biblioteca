...
BEGIN
  -- 2. Buscamos qué libro se está devolviendo
  SELECT id_libro INTO v_id_libro
  FROM prestamos
  WHERE id_prestamos = p_id_prestamo;

  -- 3.Actualizamos el estado del préstamo
  UPDATE prestamos
  SET estado = 'Devuelto'
  WHERE id_prestamos = p_id_prestamo;

  -- 4.Sumamos 1 a la cantidad disponible de ese libro
  UPDATE libros
  SET cantidad_disponible = cantidad_disponible + 1
  WHERE id_libro = v_id_libro;

  -- 5. Guardamos los cambios permanentemente
  COMMIT;
